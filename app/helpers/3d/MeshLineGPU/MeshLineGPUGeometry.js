...
export default class MeshLineGPUGeometry extends THREE.BufferGeometry {
  ...
  constructor(linesCount, lineDivision, textureWidth, textureHeight, offsetID = 0) {
    ...
    let aID = new THREE.BufferAttribute(ID, 2)
    aID.setDynamic(true)
    let aFirstID = new THREE.BufferAttribute(FirstID, 2)
    aFirstID.setDynamic(true)
    let aPrev = new THREE.BufferAttribute(prevID, 2)
    aPrev.setDynamic(true)
    let aNext = new THREE.BufferAttribute(nextID, 2)
    aNext.setDynamic(true)
    let aUV = new THREE.BufferAttribute(uv, 2)
    aUV.setDynamic(true)
    let aSide = new THREE.BufferAttribute(side, 1)
    aSide.setDynamic(true)
    let aColor = new THREE.BufferAttribute(color, 3)
    aColor.setDynamic(true)
    this.addAttribute('ID', aID)
    this.addAttribute('FirstID', aFirstID)
    this.addAttribute('prevID', aPrev)
    this.addAttribute('nextID', aNext)
    this.addAttribute('uv', aUV)
    this.addAttribute('side', aSide)
    this.addAttribute('color', aColor)

    let aIndex = new THREE.BufferAttribute(index, 1)
    aIndex.setDynamic(true)
